...
`on` is one of four rules:

Rule | Use | Example
:--- | :--- | :---
`onDate(month, day)` | Same date every year | `onDate(3, 14)` for Mar 14
`nthWeekday(n, weekday, month)` | "2nd Sunday of May" type | `nthWeekday(2, 0, 5)` for Mother's Day
`hebrewDate(monthName, day, offset)` | A Hebrew calendar date | `hebrewDate("Nisan", 15, -1)` for Passover, from the evening before
`lunarNewYear` | Lunar New Year | Already in the list
